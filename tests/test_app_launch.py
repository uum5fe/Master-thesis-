...
def test_banner_in_a_notebook_says_so_when_there_is_no_proxy(monkeypatch):
    ...
    # This branch is the local-only dashboard, so the advice a notebook
    # user gets is to run it on their own machine, not to deploy it.
...
def test_the_hint_says_what_to_do_when_nothing_is_configured(monkeypatch):
    # Reloading settings re-reads .env, which would put the developer's own
    # paths straight back into the environment this test just cleared - so the
    # test passed or failed depending on whose checkout it ran in.
    monkeypatch.setenv("EIS_SKIP_DOTENV", "1")
    for key in ("EIS_RESULTS_ROOT", "EIS_FAMOS_ROOT", "EIS_CSV_ROOT",
                "EIS_CALIBRATION_ROOT",
