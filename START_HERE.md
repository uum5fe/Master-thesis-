...
**This branch is the dashboard, and only the dashboard.** The Databricks
notebook and its copy of the pipeline are not here: this is a repository you
open in VS Code, on your own machine, and run. The bronze/silver/gold pipeline
is still bundled, in `local_eis/`, because the app can run it on raw `.DAT`
itself — but there is no notebook, no `dbutils`, and no `/Volumes` path
anywhere.

## Open it in VS Code

1. `File → Open Folder…` and pick this folder — **this folder**, not a parent.
   Everything below assumes it is the workspace root.
...
3. `Ctrl+Shift+P → Python: Select Interpreter →` the `.venv` you just made.
4. Copy `.env.example` to `.env`. It already carries the share paths; edit it
   if yours differ. **This is the one place a path is written down** — no
   Python file in the project contains one.
5. `F5`, or the Run and Debug panel, and pick a configuration:

   | Configuration | What it does |
   | --- | --- |
   | **Dashboard** | starts the app; the link is printed before the server comes up |
   | Dashboard (open browser, no reloader) | same, opens a browser, no auto-reload — use this when debugging, the reloader restarts the process out from under a breakpoint |
   | Evaluate one condition | turns raw `.DAT` into results, staging each card to a local disk first |
   | Tests | the whole suite |

### One setting worth changing before the first long run

The recordings live on a network share. The FAMOS reader memory-maps each card
and bronze walks it repeatedly, so over SMB every page fault is a network round
trip — a run that takes minutes locally takes hours. Uncomment `EIS_STAGE_DIR`
in `.env` and point it at a local disk; each card is then copied there once
before being processed.

## The plate map, drawn as the plate actually is

There are now two plate tabs, and they answer different questions.

| Tab | What it draws |
| --- | --- |
| **Plate map** | the fast grid view — a value per segment, laid out on a rectangular lattice |
| **Plate (realistic)** | the true copper: 72 segments over a 45 x 20 pad grid, each segment the exact SET OF PADS carrying its number, staircase boundaries and all |

The realistic view is worth the extra second it takes to draw, because the
segments are not the same size — they run from 1.355 to 8.470 cm2, a factor of
6.2 — and a lattice draws them as if they were. A gradient that looks smooth on
the lattice can be a strong edge on the copper, and the other way round.

It carries the things the geometry makes possible and the lattice does not:
both numbering schemes (as wired, and top-left-down) switchable with every
hover showing both, cathode/anode side, the flow channels, and the gas ports —
H2 enters bottom left and leaves top right, air enters bottom right and leaves
top left, so each inlet faces the other's outlet. Click a segment for its pads,
area, centroid and shape.

`app/plates/geometry/` holds the model. `PLATE.self_check()` asserts that the
72 segments tile all 900 pads exactly once and that the areas sum to
304.92 cm2; that check runs in the test suite, because a pad matrix with a typo
draws a heat map that looks entirely plausible and is wrong.

### A plate map you can email

The same geometry writes one self-contained HTML file — every control, no
server, no network, no dependencies:

```python
from app.plates.geometry import Field, write_html

fields = [Field("j",   "Current density", "A/cm2",  "inferno", 3, values_j),
          Field("hfr", "HFR (Rs)",        "mOhm.cm2", "viridis", 1, values_hfr)]
write_html(fields, "out/2611976_150A.html", title="2611976 - 150 A")
```

A few hundred kB, opens anywhere, and the controls hide themselves when it is
printed.
...
python run_dashboard.py --open        # paths come from .env
```
