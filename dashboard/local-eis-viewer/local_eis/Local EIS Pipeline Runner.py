...
# These three are the only paths in this notebook. Everything else is derived
# from them or comes from a widget, so pointing the pipeline at a different
# campaign is an edit here and nowhere else.
FAMOS_ROOT = Path('/Volumes/ps_xplatform_dev/rvadvtec_dev/ev_rvadvtec_dev/Famos')

# Whole-cell Gamry .DTA sweeps (and, beside them, the bench .mf4 log) for the
# SAME cell and the same operating points. Used only for the cross-check: the
# 72 segments in parallel must reproduce these. Leave blank to skip it.
GAMRY_ROOT = Path('/Volumes/ps_xplatform_dev/rvadvtec_dev/ev_rvadvtec_dev/HFR')
try:
...
try:
    ...
    dbutils.widgets.text('gamry_dir', str(GAMRY_ROOT),
                         'Whole-cell Gamry .DTA folder (blank = skip check)')
    dbutils.widgets.text('bench_log', '',
                         'Bench .mf4 log (blank = find one in the Gamry folder)')
except Exception:
    pass
...
GAMRY_DIR = _w('gamry_dir', str(GAMRY_ROOT)).strip()
BENCH_LOG = _w('bench_log').strip()
