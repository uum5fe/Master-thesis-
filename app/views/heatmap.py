...
SCALE_MODES = [
    {"label": "Robust (2–98 %)", "value": "robust"},
    ...
    return html.Div([
        ui.panel([
            html.Div([
                ...
                html.Div(ui.field("Colour limits",
                                  dcc.Dropdown(id="hm-scale", options=SCALE_MODES,
                                               value="robust", clearable=False),
                                  "Robust limits stop one bad segment from "
                                  "flattening the whole map."),
                         style={"flex": "1 1 180px"}),
                html.Div(ui.field("Min",
                                  dcc.Input(id="hm-vmin", type="number",
                                            debounce=True, style={"width": "100%"})),
                         style={"flex": "1 1 100px"}),
                html.Div(ui.field("Max",
                                  dcc.Input(id="hm-vmax", type="number",
                                            debounce=True, style={"width": "100%"})),
                         style={"flex": "1 1 100px"}),
...
def register(app):
    ...
    @app.callback(Output("hm-map", "figure"), Output("hm-dist", "figure"),
                  ...
                  Input("hm-scale", "value"), Input("hm-vmin", "value"),
                  Input("hm-vmax", "value"), Input("hm-labels", "value"))
    def _map(selection, param, scale, vmin, vmax, labels):
        ...
        fig = plate_heatmap(
            ...
            faults=faults, vmin=lo, vmax=hi, robust=(scale == "robust"),
