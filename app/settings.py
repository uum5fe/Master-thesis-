...
Put them in a ``.env`` file next to ``run_dashboard.py`` - copy
``.env.example`` and edit it.  That file is the one place a path is ever written down; no module in this
...
def load_dotenv(path: str | Path | None = None) -> str:
    ...
    # An explicit opt-out. A container configured purely from the environment
    # does not want a .env that happened to arrive on a bind mount silently
    # overriding nothing-in-particular, and a test that clears the environment
    # needs the clearing to survive a module reload.
    if os.environ.get("EIS_SKIP_DOTENV", "").strip().lower() in ("1", "true", "yes"):
        DOTENV_LOADED = ""
        return ""
...
class Settings:
    ...
    #: One or more roots holding R2-D2 CSV logger sweeps -- folders of
    #: ``metadata.csv`` + ``p1.csv, p2.csv, ...``.  Falls back to the FAMOS and
    #: results roots so a sweep sitting beside them is found without extra
    #: configuration.
    csv_roots: list[str] = field(
        default_factory=lambda: _env_list("EIS_CSV_ROOT"))
