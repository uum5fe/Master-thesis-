...
# ---------------------------------------------------------------------------
# the safety net under the per-rate grouping
# ---------------------------------------------------------------------------

def test_a_mixed_rate_selection_is_refused_not_correlated(tmp_path) -> None:
    """The last line of defence for `evaluate_per_card --group rate`.

    Grouping by rate is what keeps two clocks out of one run; this is what
    happens if a group ever leaks one anyway. It must be a refusal and not a
    lag, because a lag in samples is a different amount of time on each card
    and NOTHING downstream can tell that it is wrong -- by the time silver
    sees it, the only symptom is every segment of one card failing an SNR
    gate, which is what a wrong lag always looks like.

    `--no-require-timebase` deliberately does not reach this. That flag lets
    a run past a timebase whose STAMPS disagree, which is a judgement call an
    operator may reasonably overrule; a lag that cannot be expressed is not.
    """
    from pathlib import Path

    fs_fast, fs_slow = 100_000.0, 50_000.0

    files, cards = [], {}
    for stem, fs in (("fast", fs_fast), ("slow", fs_slow)):
        n_s = int(2.0 * fs)
        p = tmp_path / f"{stem}.DAT"
        p.write_bytes(b"")
        files.append(p)
        cards[stem] = B.CardInfo(path=p, stem=stem, fs=fs, n_ch=16,
                                 n_samples=n_s, duration_s=2.0,
                                 ref_name="UC1", ref_slot=0, n_segments=15)

    cfg = DEFAULT.replace(require_timebase=False)
    with pytest.raises(SystemExit) as excinfo:
        B.estimate_card_lags(files, cards, cfg)
    message = str(excinfo.value)
    assert "sample rate" in message
    assert "100000" in message and "50000" in message, \
        "the message must name the rates, so the split is obvious"
