...
def run_pipeline(cfg: Config, stop_after: str = "gold") -> dict:
    ...
    manifest = {"config": cfg.to_dict(), "stages": {}}
...
def self_test() -> int:
    ...
    ok = len(geom.SEGMENTS) == 72
    log.info(f"  geometry: {len(geom.SEGMENTS)} segments, "
             f"areas {min(geom.areas().values()):.3f}"
             f"..{max(geom.areas().values()):.3f} cm2   "
